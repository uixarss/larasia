    ...
    <div class="card">
        <div class="card-header border-0">
            <ul class="nav nav-stretch nav-line-tabs nav-line-tabs-2x border-transparent fs-5 fw-bold">
                ...
                <li class="nav-item">
                    <a class="nav-link text-active-primary py-5" data-bs-toggle="tab" href="#rekap">Rekap Periode</a>
                </li>
            </ul>
        </div>
        <div class="card-body">
            <div class="tab-content">
                <div class="tab-pane fade" id="harian" role="tabpanel">
                    ...
                    <table id="table_data"
                        ...
                        <tbody class="fs-6 fw-semibold text-gray-600">
                            @foreach ($absensi_dosen as $dosen)
                                <tr>
                                    <td>{{ $dosen->dosen->nidn }}</td>
                                    <td>{{ $dosen->dosen->nama_dosen }}</td>
                                    <td>{{ $dosen->mapel->nama_mapel }}</td>
                                    <td>{{ $dosen->kelas->nama_kelas }}</td>
                                    <td>{{ $dosen->pertemuan_ke }}</td>
                                    <td>{{ $dosen->status }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                ...
                <div class="tab-pane fade show active" id="laporan" role="tabpanel">
                    <div class="d-flex flex-wrap flex-stack mb-5">
                        ...
                        <div class="d-flex align-items-center">
                                <a href="{{ route('admin.absensi.dosen.laporan', ['tanggal_absen' => $tanggal_absen]) }}" class="btn btn-sm btn-success btn-icon">
                                    <span class="fa fa-cloud-download"></span>
                                </a>
                                <div class="bullet bg-secondary h-35px w-1px mx-5"></div>
                                <form action="{{ route('admin.absensi.dosen.cari') }}" method="POST" class="d-flex my-2">
                                    @csrf
                                    <div class="row g-3">
                                        <div class="col-auto">
                    ...
                    <table id="table_laporan"
                        ...
                        <tbody class="fs-6 fw-semibold text-gray-600">
                            @foreach ($absensi_dosen_tanggal as $dosen)
                                <tr>
                                    <td>{{ $dosen->dosen->nidn }}</td>
                                    <td>{{ $dosen->dosen->nama_dosen }}</td>
                                    <td>{{ $dosen->status }}</td>
                ...
                <div class="tab-pane fade" id="rekap" role="tabpanel">
                    <div class="d-flex flex-wrap flex-stack mb-5">
                        <h3 class="d-flex flex-column justify-content-center flex-wrap mb-0">
                            <span class="card-label fw-bold fs-3 mb-2">Rekap Absensi</span>
                            <span class="text-muted fw-semibold fs-7">{{ \Carbon\Carbon::parse($start ?? '')->format('d M Y') }} - {{ \Carbon\Carbon::parse($end ?? '')->format('d M Y') }}</span>
                        </h3>
                        ...
                        <div class="d-flex align-items-center">
                                @if ($start ?? ('' != null && $end ?? '' != null))
                                    ...
                                    <div class="bullet bg-secondary h-35px w-1px mx-5"></div>
                                @endif
                                <form action="{{ route('admin.absensi.dosen.cari.rekap') }}" method="POST" class="d-flex my-2">
                                    @csrf
                                    <div class="row g-3">
                                        <div class="col-auto">
                                        ...
                                        <div class="col-auto">
                                            <button type="submit" class="btn btn-primary"><i class="bi bi-search"></i>Rekap</button>
                                        </div>
                                    </div>
                                </form>
                        </div>
                    ...
                    <table id="table_rekap"
                        ...
                        <tbody class="fs-6 fw-semibold text-gray-600">
                            ...
                            @foreach ($data_absensi as $key => $absensi)
                                <tr>
                                    <td>{{ ++$key }}</td>
                                    <td>{{ $absensi->mapel->kode_mapel ?? '' }}</td>
                                    <td>{{ $absensi->mapel->nama_mapel ?? '' }}</td>
                                    <td>{{ $absensi->dosen->nidn }}</td>
                                    <td>{{ $absensi->dosen->nama_dosen ?? '' }}</td>
...
@section('data-scripts')
    ...
    <script>
        ...
        var ListLaporan = function() {
            var table = document.getElementById('table_laporan');
            var datatable;
            var toolbarBase;
