...
@section('content')
    <div class="card card-flush h-lg-100">
        ...
        <div class="card-body">
            <table id="table_data" class="table align-middle table-rounded border table-row-bordered table-row-gray-100 bg-white gs-5 gy-4">
                ...
                <tbody>
                    @foreach ($kebutuhan_khusus as $key => $kebutuhan)
                    <tr>
                        <td>{{ ++$key }}</td>
                        <td>{{ $kebutuhan->kebutuhan_khusus }}</td>
                        <td class="text-end">
                            <a class="btn btn-sm btn-warning" data-bs-toggle="modal"
                                data-bs-target="#editKebutuhan{{ $kebutuhan->id }}">Edit</a>
                            <a href="{{ route('admin.kebutuhankhusus.destroy', $kebutuhan->id) }}"
                                class="btn btn-sm btn-danger" onclick="return confirm('Yakin Mau di Hapus ?')">Hapus</a>
                        </td>
                    </tr>
                    <!-- Modal edit prodi-->
                    <div class="modal fade" id="editKebutuhan{{ $kebutuhan->id }}" data-backdrop="static" tabindex="-1"
                        role="dialog" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h3 class="modal-title">Edit Kebutuhan Khusus</h3>
                                ...
                                <div class="modal-body">
                                    ...
                                    <form action="{{ route('admin.kebutuhankhusus.update', $kebutuhan->id) }}"
                                        method="post">
                                        @csrf
                                        ...
                                        <div class="fv-row">
                                            ...
                                            <input name="kebutuhan_khusus" type="text" class="form-control"
                                                value="{{ $kebutuhan->kebutuhan_khusus }}">
                                        </div>
    ...
    <div class="modal fade" tabindex="-1" id="tambahpengumuman">
        <div class="modal-dialog">
            <form class="modal-content" action="{{ route('admin.kebutuhankhusus.create') }}" method="post">
                <div class="modal-header">
                    <h3 class="modal-title">Tambah Kebutuhan Khusus</h3>
