...
@section('title-page')
    ...
    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
        ...
        <li class="breadcrumb-item text-muted">Jenis Pekerjaan</li>
    </ul>
@endsection
...
@section('content')
    <div class="card card-flush h-lg-100">
        ...
        <div class="card-body">
            <table id="table_data" class="table align-middle table-rounded border table-row-bordered table-row-gray-100 bg-white gs-5 gy-4">
                ...
                <tbody>
                    @foreach ($jenis_pekerjaan as $key => $pekerjaan)
                        <tr>
                            <td>{{ ++$key }}</td>
                            <td>{{ $pekerjaan->jenis_pekerjaan }}</td>
                            <td class="text-end">
                                <a class="btn btn-sm btn-warning" data-bs-toggle="modal"
                                    data-bs-target="#editPekerjaan{{ $pekerjaan->id }}">Edit</a>
                                <a href="{{ route('admin.jenispekerjaan.destroy', $pekerjaan->id) }}"
                                    class="btn btn-sm btn-danger" onclick="return confirm('Yakin Mau di Hapus ?')">Hapus</a>
                            </td>
                        </tr>
                        <!-- Modal edit prodi-->
                        <div class="modal fade" id="editPekerjaan{{ $pekerjaan->id }}" data-backdrop="static" tabindex="-1"
                            role="dialog" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h3 class="modal-title">Edit Jenis Pekerjaan</h3>
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <form action="{{ route('admin.jenispekerjaan.update', $pekerjaan->id) }}"
                                            method="post">
                                            @csrf
    ...
    <div class="modal fade" tabindex="-1" id="tambahpengumuman">
        <div class="modal-dialog">
            <form class="modal-content" action="{{ route('admin.jenispekerjaan.create') }}" method="post">
                <div class="modal-header">
                    <h3 class="modal-title">Tambah Jenis Pekerjaan</h3>
